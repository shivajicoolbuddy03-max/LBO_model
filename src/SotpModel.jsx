...
For share price specifically, you may ALSO cite the primary listing exchange's own site (e.g. nyse.com, nasdaq.com, or the relevant national exchange's own domain) if the company's own IR site doesn't have it.
...
❌ Only these count as prohibited: pages on a THIRD-PARTY financial site's own domain — Yahoo Finance, Google Finance, Bloomberg.com, Reuters, MarketWatch, Simply Wall St, stockanalysis.com, wsj.com, Macrotrends, TradingView, screener.in, moneycontrol, Wisesheets, or any other stock screener/aggregator/news article/broker note.
...
If a figure genuinely only appears on a prohibited domain, set "value" to JSON null. Do not reason about it further. For every field, give the exact source document/page name, the URL, and the as-of or filing date, each kept short.
...
function sharePriceSourcingHint(companyLabel) {
  return `Check the company's own investor-relations website under its "Stock Information" / "Stock Quote & Chart" section. This page belongs to ${companyLabel} itself, so it is fully permitted — a "delayed quote" or "data provided by [vendor]" disclaimer does NOT make it prohibited. If that page's number isn't extractable as plain text, try: (a) the primary listing exchange's own quote page for this ticker; (b) the company's most recent insider-transaction filing on the official regulator system, which discloses an exact transaction price and date in a plain-text table — a transaction within the last month or two is a reasonable proxy for "current" price. If none of these yield a plain-text number, set the value to null.`;
}
