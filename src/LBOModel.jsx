...
const SOURCE_RULE = `SOURCING RULE, NON-NEGOTIABLE. Every figure must come from the company's own primary disclosure: the audited annual report, the interim or quarterly filing, the results announcement, or the investor presentation, retrieved from the company's investor-relations site or the regulator of the exchange it is listed on. ${FILING_SOURCES.map((f) => `${f[0]}: ${f[1]} (${f[2]})`).join(". ")}. Never take a figure from a data aggregator, a screener, a broker note, an encyclopedia or a news article. If the primary filing cannot be found, return the field as 0 and say so in the source note rather than substituting an estimate. EXCEPTION, market_cap_mm only: because market capitalisation is a continuously-quoted price times shares outstanding rather than a periodic disclosure, source it from the company's IR "stock info" page or the listing exchange's own quote page first; only if neither yields a plain number, a reputable third-party quote service — Bloomberg, Yahoo Finance, or Google Finance — may be used as a last resort, and source_note must say so explicitly (e.g. "market cap via Yahoo Finance, third-party — IR/exchange quote unavailable"). This exception does not extend to any other field.`;
 ...
 * XLSX writer (StyleBook/WSheet/writeXlsx/colName) is the shared
 * module imported above — same raw-OOXML approach both other tools
 * use, with the leading-'=' and null-cached-value fixes applied.
 * ------------------------------------------------------------------ */
const XP = {
...
function buildWorkbook(d) {
  ...
  dash.fmlStr(4, 9, `IF(${Rt(RT.irr)}>=${A(AS.hurdle)},"Clears the hurdle",IF(${Rt(RT.irr)}>=${A(AS.hurdle)}-0.05,"Marginal against the hurdle","Below the hurdle"))`, m.verdict.label, S.lkStrBlk);
